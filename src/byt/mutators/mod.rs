//! ## Introduction
//! Mutators are byt's strategy for allowing extension of the editor's functionality. In fact, byt
//! can't do anything without plugins. Its base functionality is really just a set of plugins that
//! provide typical editor workflows, like vym, which implements an interesting subset of vim
//! functionality.
//!
//! byt uses an embedded language called gluon for all of its plugins. It is a statically typed,
//! functional programming language that allows for side effects. By default byt executes all gluon
//! code with the run_io flag so that the user doesn't have to worry about switching between code
//! with and without side effects.
//!
//! ## Operation
//! Mutators can be attached either to the editor itself or a particular type of view, which for
//! right now is just FileViews (akin to vim's buffers.) Mutators that attach to the editor are
//! effectively global; they can intercept and modify just about everything in the editor. This may
//! not be desirable for all mutators, thus we provide the FileView attachment mechanism. Each
//! FileView that is initialized (e.g by opening a file) gets its own instance of a particular
//! mutator. They do not share state across FileView boundaries, only editor-level mutators do. In
//! fact, each FileView has its own gluon virtual machine.
